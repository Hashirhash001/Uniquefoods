...
<style>
    ...
    /* Items table */
    .oe-table { width: 100%; border-collapse: collapse; font-size: 13px; min-width: 520px; }
    ...
    .qty-input { width: 72px; border: 1px solid #d1d5db; border-radius: 6px; padding: 6px 8px; font-size: 13px; text-align: center; font-family: inherit; }
    .price-input { width: 96px; border: 1px solid #d1d5db; border-radius: 6px; padding: 6px 8px; font-size: 13px; font-family: inherit; }
    .qty-input:focus, .price-input:focus { border-color: #08437b; outline: none; box-shadow: 0 0 0 2px rgba(8,67,123,0.08); }
            ...
            <div class="oe-card-body">
                <div class="oe-grid" style="margin-bottom:16px;">
                    <div>
                        ...
                        <input type="text" name="customer_name" class="oe-input"
                               value="{{ $order->customer_name }}" required>
                    </div>
                    <div>
                        <label class="oe-label">Email <span style="color:#ef4444;">*</span></label>
                        <input type="email" name="customer_email" class="oe-input"
                               value="{{ $order->customer_email }}" required>
                    </div>
                </div>
                <div>
                    <label class="oe-label">Phone</label>
                    <input type="text" name="customer_phone" class="oe-input" style="max-width:260px;"
        ...
        <div class="oe-card">
            ...
            <div class="oe-card-body">
                <div style="margin-bottom:16px;">
                    ...
                    <input type="text" name="shipping_address" class="oe-input"
                           value="{{ $order->shipping_address }}" required>
                </div>
                <div class="oe-grid-3">
                    <div>
                        <label class="oe-label">City</label>
                        <input type="text" name="shipping_city" class="oe-input"
                               value="{{ $order->shipping_city }}">
                    </div>
                    <div>
                        <label class="oe-label">Postcode</label>
                        <input type="text" name="shipping_postcode" class="oe-input"
                               value="{{ $order->shipping_postcode }}">
                    </div>
                    <div>
                        <label class="oe-label">Country</label>
                        <input type="text" name="shipping_country" class="oe-input"
...
// Delegate live recalc to existing rows
...
searchResults.addEventListener('click', e => {
    ...
    if (!item) return;
    addRow(item.dataset.id, item.dataset.name, item.dataset.sku, item.dataset.price);
    searchInput.value = '';
...
// ─── ADD ROW ─────────────────────────────────────────────────────────────────
function addRow(productId, name, sku, price) {
    ...
    tr.innerHTML = `
        ...
        <td><input type="number" min="1" class="qty-input item-qty" value="1"></td>
    ...
    .then(data => {
        ...
        if (data.success) {
            Swal.fire({
                icon: 'success',
                title: 'Order Updated!',
                html: `${data.message}<br><small style="color:#6b7280;">Customer has been notified by email with the new invoice.</small>`,
