            ...
            <form id="productForm" enctype="multipart/form-data">
                    ...
                    <div class="row g-4">
                        <div class="col-md-8">
                            ...
                            <input type="text" name="name" class="form-control"
                                   placeholder="e.g., Fresh Tomatoes, Basmati Rice 5kg"
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="is_weight_based" class="form-select" id="weightToggle">
                                <option value="0" {{ old('is_weight_based', $product->is_weight_based) == 0 ? 'selected' : '' }}>No - Fixed Price</option>
                                <option value="1" {{ old('is_weight_based', $product->is_weight_based) == 1 ? 'selected' : '' }}>Yes - Price per KG</option>
                            </select>
                ...
                <div class="form-section">
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-3 {{ old('is_weight_based', $product->is_weight_based) ? '' : 'd-none' }}" id="minWeightWrapper">
                            <label class="form-label">Min Weight (kg)</label>
                            <input type="number" step="0.001" name="min_weight" class="form-control"
                                   placeholder="0.250" value="{{ old('min_weight', $product->min_weight) }}">
                            <small class="text-danger error-min_weight"></small>
                        </div>

                        <div class="col-md-3 {{ old('is_weight_based', $product->is_weight_based) ? '' : 'd-none' }}" id="maxWeightWrapper">
                            <label class="form-label">Max Weight (kg)</label>
                            <input type="number" step="0.001" name="max_weight" class="form-control"
                                   placeholder="5.000" value="{{ old('max_weight', $product->max_weight) }}">
                ...
                <div class="form-section">
                    ...
                    <div class="row g-3 mb-3" id="existingImages">
                        ...
                        <div class="col-md-2 col-6" data-image-id="{{ $image->id }}">
                            <div class="preview-card">
                                <span class="preview-remove" onclick="removeExistingImage({{ $image->id }})">×</span>
...
function handleFiles(files) {
    ...
    // Check total count
    const existingCount = typeof existingImagesCount !== 'undefined' ? existingImagesCount - deletedImageIds.length : 0;
    const totalImages = existingCount + selectedFiles.length + files.length;

    if (totalImages > MAX_FILES) {
        showImageError(`Maximum ${MAX_FILES} images allowed. You already have ${existingCount + selectedFiles.length} image(s).`);
        return;
    }
        ...
        reader.onload = e => {
            ...
            col.innerHTML = `
                <div class="preview-card">
                    ...
                    ${(existingImagesCount - deletedImageIds.length === 0 && index === 0) ? '<div class="badge">Primary</div>' : ''}
                </div>`;
        ...
        error: function(xhr) {
            ...
            if (xhr.status === 422) {
                ...
                Object.entries(errors).forEach(([field, messages]) => {
                    // Handle array-based errors (like images.0, images.1)
                    const cleanField = field.replace(/\.\d+$/, ''); // Remove .0, .1, etc.
                    const errorElement = $(`.error-${cleanField}`);
                    const inputElement = $(`[name="${field}"], [name="${cleanField}"], [name="${cleanField}[]"]`);

                    if (errorElement.length) {
                        // Show first error in the error element
